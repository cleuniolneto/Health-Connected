...
const FacilityAudit = require("./models/facilityAudit");
...
const DEFAULT_FACILITIES = [
  {
    ...
    tipo: "hospital",
    endereco: "Luanda",
    contacto: "+244 900 000 001",
    lat: -8.839,
    lng: 13.2894,
    especialidades: ["Pediatria", "Cardiologia", "Clinica Geral"],
    medicos: [
      { nome: "Dr. Paulo Mendes", especialidade: "Cardiologia" },
      { nome: "Dra. Ana Costa", especialidade: "Pediatria" }
    ]
  },
  {
    nome: "Clinica Esperanca",
    tipo: "hospital",
    endereco: "Benfica, Luanda",
    contacto: "+244 900 000 002",
    lat: -8.92,
    lng: 13.19,
    especialidades: ["Pediatria", "Dentista"],
    medicos: [
      { nome: "Dr. Ivo Santos", especialidade: "Dentista" }
    ]
  },
  {
    nome: "Posto Medico Mutamba",
    tipo: "posto",
    endereco: "Mutamba, Luanda",
    contacto: "+244 900 000 010",
    lat: -8.815,
    lng: 13.23,
    especialidades: ["Clinica Geral"],
    medicos: [
      { nome: "Enf. Joana Lima", especialidade: "Clinica Geral" }
    ]
  },
  {
    nome: "Farmacia Popular Luanda",
    tipo: "farmacia",
    endereco: "Mutamba, Luanda",
    contacto: "+244 900 000 100",
    lat: -8.815,
    lng: 13.23,
    farmacos: [
      { nome: "Paracetamol", preco: 450, disponibilidade: "disponivel", entregaDomicilio: true },
      { nome: "Amoxicilina", preco: 1200, disponibilidade: "encomenda", entregaDomicilio: false },
      { nome: "Insulina", preco: 3500, disponibilidade: "disponivel", entregaDomicilio: true }
    ]
  },
  {
    nome: "Farmacia Saúde+",
    ...
    contacto: "+244 900 000 120",
    lat: -8.88,
    lng: 13.22,
    farmacos: [
      { nome: "Paracetamol", preco: 500, disponibilidade: "disponivel", entregaDomicilio: false },
      { nome: "Vitamina C", preco: 800, disponibilidade: "disponivel", entregaDomicilio: true }
    ]
  }
];
...
function requireProfessionalApproved(req, res, next) {
  if (req.auth && req.auth.role !== "profissional") return next();
  if (req.auth && req.auth.role === "profissional") {
    const status = req.currentUser && req.currentUser.profissionalStatus;
    if (status !== "aprovado") {
...
function suggestSpecialtyFromTriage(respostas = {}) {
  const sintomas = String(respostas.sintomas || "").toLowerCase();
  if (sintomas.includes("dente")) return "Dentista";
  if (sintomas.includes("cora") || sintomas.includes("peito")) return "Cardiologia";
  if (sintomas.includes("ansiedade") || sintomas.includes("estresse") || sintomas.includes("triste")) return "Psicologia";
  if (sintomas.includes("olho") || sintomas.includes("visao")) return "Oftalmologia";
  if (String(respostas.idade || "").trim() && Number(respostas.idade) <= 12) return "Pediatria";
  return "Clinica Geral";
}

function buildTriageResumo(respostas = {}) {
  const sintomas = String(respostas.sintomas || "").trim();
  const duracao = String(respostas.duracao || "").trim();
  const intensidade = String(respostas.intensidade || "").trim();
  const temperatura = String(respostas.temperatura || "").trim();
  const idade = String(respostas.idade || "").trim();
  return [
...
app.post("/auth/bootstrap-gestor", async (req, res, next) => {
  try {
    const bootstrapToken = process.env.ADMIN_BOOTSTRAP_TOKEN;
    const provided = String(req.headers["x-admin-bootstrap-token"] || req.body.token || "");
    if (!bootstrapToken || provided !== bootstrapToken) {
      return res.status(403).json({ ok: false, error: "Token de bootstrap invalido" });
    }

    const existingGestor = await User.findOne({ role: "gestor" }).select("_id");
    if (existingGestor) return res.status(409).json({ ok: false, error: "Ja existe um gestor criado" });
    ...
    return res.status(201).json({ ok: true, gestorId: gestor._id, message: "Gestor inicial criado" });
  } catch (err) {
    return next(err);
  }
});
...
app.post("/api/gestor/usuarios/:id/promover", authMiddleware, requireAppAccess, requireRoles("gestor"), async (req, res, next) => {
  try {
    const user = await User.findById(req.params.id);
    if (!user) return res.status(404).json({ ok: false, error: "Usuario nao encontrado" });
    if (user.role === "gestor") return res.json({ ok: true, message: "Usuario ja e gestor" });
    if (user.emailVerificado === false) return res.status(400).json({ ok: false, error: "Email do usuario nao verificado" });
    user.role = "gestor";
    await user.save();
    return res.json({ ok: true, message: "Usuario promovido a gestor" });
  } catch (err) {
    return next(err);
  }
});

app.post("/api/gestor/usuarios/:id/rebaixar", authMiddleware, requireAppAccess, requireRoles("gestor"), async (req, res, next) => {
  try {
    if (String(req.userId) === String(req.params.id)) {
      return res.status(400).json({ ok: false, error: "Nao e possivel rebaixar o proprio usuario" });
    }
    const user = await User.findById(req.params.id);
    if (!user) return res.status(404).json({ ok: false, error: "Usuario nao encontrado" });
    if (user.role !== "gestor") return res.status(400).json({ ok: false, error: "Usuario nao e gestor" });
    const totalGestores = await User.countDocuments({ role: "gestor" });
    if (totalGestores <= 1) {
      return res.status(400).json({ ok: false, error: "Nao e possivel rebaixar o ultimo gestor" });
    }
    user.role = "paciente";
    await user.save();
    return res.json({ ok: true, message: "Gestor rebaixado para paciente" });
  } catch (err) {
    return next(err);
  }
});
...
app.post("/api/facilities", authMiddleware, requireAppAccess, requireRoles("gestor"), async (req, res, next) => {
  try {
    const { nome, tipo, endereco, contacto, lat, lng, especialidades, medicos, farmacos } = req.body;
    if (!nome || !tipo || lat === undefined || lng === undefined) return res.status(400).json({ ok: false, error: "nome, tipo, lat e lng sao obrigatorios" });

    const facility = await Facility.create({ nome: String(nome).trim(), tipo, endereco: String(endereco || "").trim(), contacto: String(contacto || "").trim(), lat: Number(lat), lng: Number(lng), especialidades: Array.isArray(especialidades) ? especialidades : [], medicos: Array.isArray(medicos) ? medicos : [], farmacos: Array.isArray(farmacos) ? farmacos : [] });
...
app.put("/api/facilities/:id", authMiddleware, requireAppAccess, requireRoles("gestor"), async (req, res, next) => {
  try {
    const facility = await Facility.findById(req.params.id);
    if (!facility) return res.status(404).json({ ok: false, error: "Local nao encontrado" });

    const before = facility.toObject();

    const allowed = ["nome", "tipo", "endereco", "contacto", "lat", "lng", "especialidades", "medicos", "farmacos", "ativo"]; 
    for (const key of allowed) {
      if (req.body[key] === undefined) continue;
      if (key === "lat" || key === "lng") facility[key] = Number(req.body[key]);
      else if (key === "especialidades") facility.especialidades = Array.isArray(req.body.especialidades) ? req.body.especialidades : [];
      else if (key === "medicos") facility.medicos = Array.isArray(req.body.medicos) ? req.body.medicos : [];
      else if (key === "farmacos") facility.farmacos = Array.isArray(req.body.farmacos) ? req.body.farmacos : [];
      else facility[key] = req.body[key];
    }

    await facility.save();

    await FacilityAudit.create({
      facilityId: facility._id,
      action: "facility_update",
      before,
      after: facility.toObject(),
      actorId: req.userId,
      actorNome: req.auth?.nome || "",
...
app.delete("/api/facilities/:id", authMiddleware, requireAppAccess, requireRoles("gestor"), async (req, res, next) => {
  try {
    const facility = await Facility.findById(req.params.id);
    if (!facility) return res.status(404).json({ ok: false, error: "Local nao encontrado" });
    const before = facility.toObject();
    facility.ativo = false;
    await facility.save();
    ...
    return res.json({ ok: true, message: "Local desativado" });
  } catch (err) {
    return next(err);
  }
});
app.put("/api/facilities/:id/farmacos", authMiddleware, requireAppAccess, requireRoles("gestor"), async (req, res, next) => {
  try {
    const facility = await Facility.findById(req.params.id);
    if (!facility) return res.status(404).json({ ok: false, error: "Local nao encontrado" });
    if (facility.tipo !== "farmacia") return res.status(400).json({ ok: false, error: "Apenas farmacias podem ter farmacos" });

    const before = { farmacos: facility.farmacos || [] };

    const farmacos = Array.isArray(req.body.farmacos) ? req.body.farmacos : [];
    facility.farmacos = farmacos.map((m) => ({
      nome: String(m.nome || "").trim(),
      preco: Number(m.preco || 0),
      disponibilidade: ["disponivel", "esgotado", "encomenda"].includes(m.disponibilidade) ? m.disponibilidade : "disponivel",
      entregaDomicilio: Boolean(m.entregaDomicilio)
    })).filter((m) => m.nome);
    ...
    await FacilityAudit.create({
      facilityId: facility._id,
      action: "farmacos_update",
      before,
      after: { farmacos: facility.farmacos },
      actorId: req.userId,
      actorNome: req.auth?.nome || "",
      actorEmail: req.auth?.email || ""
    });
...
app.get("/api/locais/search", authMiddleware, requireAppAccess, async (req, res, next) => {
  try {
    ...
    const medico = String(req.query.medico || "").trim().toLowerCase();
    const farmaco = String(req.query.farmaco || "").trim().toLowerCase();
    const entrega = String(req.query.entrega || "").trim().toLowerCase();
    ...
    let facilities = await Facility.find(query).lean();

    if (especialidade) {
      facilities = facilities.filter((f) => (f.especialidades || []).some((e) => String(e).toLowerCase().includes(especialidade)) || (f.medicos || []).some((m) => String(m.especialidade || "").toLowerCase().includes(especialidade)));
    }

    if (medico) {
      facilities = facilities.filter((f) => (f.medicos || []).some((m) => String(m.nome || "").toLowerCase().includes(medico)));
    }

    if (farmaco) {
      facilities = facilities.filter((f) => f.tipo === "farmacia" && (f.farmacos || []).some((med) => String(med.nome || "").toLowerCase().includes(farmaco)));
    }

    if (entrega) {
      const want = entrega === "sim";
      facilities = facilities.filter((f) => f.tipo === "farmacia" && (f.farmacos || []).some((med) => Boolean(med.entregaDomicilio) === want));
    }
...
app.post("/api/consultas", authMiddleware, requireAppAccess, async (req, res, next) => {
  try {
    const { especialidade, localId, localNome, horario, triagem } = req.body;
    ...
    const triagemRespostas = triagem && typeof triagem === "object" ? triagem : {};
    const especialidadeSugerida = suggestSpecialtyFromTriage(triagemRespostas);
    const resumoTriagem = buildTriageResumo(triagemRespostas);
    const risco = (String(triagemRespostas.intensidade || "").toLowerCase() === "alta" || Number(triagemRespostas.temperatura || 0) >= 39) ? "alto" : (String(triagemRespostas.intensidade || "").toLowerCase() === "media" ? "moderado" : "baixo");
    ...
    const consulta = await Consultation.create({
      ...
      triagem: { respostas: triagemRespostas, resumo: resumoTriagem, especialidadeSugerida, risco }
    });
...
app.get("/api/consultas/pendentes/profissional", authMiddleware, requireAppAccess, requireProfessionalApproved, requireRoles("profissional"), async (req, res, next) => {
  try {
...
app.get("/api/profissional/consultas/:id", authMiddleware, requireAppAccess, requireProfessionalApproved, requireRoles("profissional"), async (req, res, next) => {
  try {
...
app.post("/api/consultas/:id/atender", authMiddleware, requireAppAccess, requireProfessionalApproved, requireRoles("profissional", "gestor"), async (req, res, next) => {
  try {
    const consulta = await Consultation.findById(req.params.id);
    if (!consulta) return res.status(404).json({ ok: false, error: "Consulta nao encontrada" });
...
app.post("/api/consultas/:id/receita", authMiddleware, requireAppAccess, requireProfessionalApproved, requireRoles("profissional", "gestor"), async (req, res, next) => {
  try {
    ...
    let assinatura = "";
    if (req.auth.role === "profissional") {
      const profUser = await User.findById(req.userId).select("nome numeroCarteira especialidade");
      assinatura = `Assinatura digital: ${profUser?.nome || "Profissional"} | Carteira: ${profUser?.numeroCarteira || "-"} | ${profUser?.especialidade || ""}`;
    } else {
      assinatura = "Assinatura digital emitida pela plataforma";
    }
    const hash = crypto.createHash("sha256").update(`${conteudo}-${consulta._id}-${Date.now()}`).digest("hex").slice(0, 16);
    ...
    await generatePdf(filePath, "Receita Medica", [`Consulta: ${consulta._id}`, `Paciente: ${consulta.pacienteId?.nome || "-"}`, `Especialidade: ${consulta.especialidade}`, "", conteudo, "", assinatura, `Hash: ${hash}`]);

    consulta.receitaPdfPath = filePath;
    consulta.receitaResumo = conteudo.slice(0, 500);
    consulta.receitaAssinatura = `${assinatura} | Hash: ${hash}`;
    await consulta.save();
...
app.post("/api/consultas/:id/receita/enviar", authMiddleware, requireAppAccess, requireProfessionalApproved, requireRoles("profissional", "gestor"), async (req, res, next) => {
  try {
    const consulta = await Consultation.findById(req.params.id).populate("pacienteId", "nome email");
    if (!consulta || !consulta.receitaPdfPath) return res.status(404).json({ ok: false, error: "Receita nao encontrada" });
...
app.get("/api/gestor/facilities/audits", authMiddleware, requireAppAccess, requireRoles("gestor"), async (req, res, next) => {
  try {
    const limit = Math.min(Number(req.query.limit || 50), 200);
    const audits = await FacilityAudit.find()
      .sort({ createdAt: -1 })
      .limit(limit)
      .lean();

    const ids = audits.map((a) => a.facilityId).filter(Boolean);
    const facilities = await Facility.find({ _id: { $in: ids } }).select("nome").lean();
    const byId = new Map(facilities.map((f) => [String(f._id), f.nome]));

    const enriched = audits.map((a) => ({
      ...a,
      facilityNome: byId.get(String(a.facilityId)) || "-"
    }));

    return res.json({ ok: true, audits: enriched });
  } catch (err) {
    return next(err);
  }
});
app.get("/api/gestor/facilities/audits.csv", authMiddleware, requireAppAccess, requireRoles("gestor"), async (req, res, next) => {
  try {
    const limit = Math.min(Number(req.query.limit || 200), 1000);
    const audits = await FacilityAudit.find().sort({ createdAt: -1 }).limit(limit).lean();
    const ids = audits.map((a) => a.facilityId).filter(Boolean);
    const facilities = await Facility.find({ _id: { $in: ids } }).select("nome").lean();
    const byId = new Map(facilities.map((f) => [String(f._id), f.nome]));

    const header = "data,acao,local,ator,ator_email\\n";
    const rows = audits.map((a) => {
      const date = new Date(a.createdAt).toISOString();
      const acao = a.action || "";
      const local = byId.get(String(a.facilityId)) || "";
      const ator = a.actorNome || "";
      const email = a.actorEmail || "";
      return `${date},${acao},${local},${ator},${email}`;
    }).join("\\n");

    res.setHeader("Content-Type", "text/csv; charset=utf-8");
    res.setHeader("Content-Disposition", "attachment; filename=auditoria-locais.csv");
    return res.send(header + rows);
  } catch (err) {
    return next(err);
  }
});
app.get("/api/gestor/facilities/audits.pdf", authMiddleware, requireAppAccess, requireRoles("gestor"), async (req, res, next) => {
  try {
    ...
    const filePath = path.join(GENERATED_DIR, `auditoria-locais-${Date.now()}.pdf`);
    const lines = audits.map((a) => {
      const date = new Date(a.createdAt).toLocaleString();
      const local = byId.get(String(a.facilityId)) || "-";
      const who = [a.actorNome || "", a.actorEmail ? `(${a.actorEmail})` : ""].join(" ").trim();
      return `${date} | ${a.action} | ${local} | ${who || "-"}`;
    });

    await generatePdf(filePath, "Historico de Alteracoes de Locais", lines);
    return res.download(filePath, "auditoria-locais.pdf");
  } catch (err) {
    return next(err);
  }
...
app.get("/api/profissionais", authMiddleware, requireAppAccess, async (req, res, next) => {
  try {
    const especialidade = String(req.query.especialidade || "").trim();
    const q = String(req.query.q || "").trim();

    const query = { role: "profissional", profissionalStatus: "aprovado" };
